...
|------|------|
| **날짜** | 2025-04-09 |
| **상태** | 승인됨 (v1 범위 축소 반영 — 아래 v1 업데이트 노트 참고) |
| **결정자** | 설계팀 전원 |
...
3. 단일 EC2 기준이지만 스케일아웃 전환 비용이 낮음

### 트레이드오프 — AccessToken 즉시 무효화 문제

AccessToken 만료 전 강제 무효화가 불가능하다. 로그아웃 시 RefreshToken만 Redis에서 삭제하고, AccessToken은 만료(15분)를 기다린다.
...
블랙리스트 구현 시:
  ...
  → Stateless JWT의 장점 상실

∴ 현재 선택: 짧은 만료시간(15분)으로 보안 위협 범위 최소화
  향후 보안 강화 필요 시 블랙리스트 도입 가능 (Redis 이미 존재)
```
...
|------|----------------|--------------|
| 인증 | JWT (변경 없음) | JWT (변경 없음) — Stateless이므로 그대로 |
| RefreshToken | Redis 단일 노드 | Redis Cluster 또는 ElastiCache Cluster Mode |
...
> ### 📌 v1 업데이트 노트 (팀원 필독)
>
> **현재 v1 구현에서 RefreshToken은 의도적으로 제외되었습니다.**
>
> | 항목 | 원래 설계 (ADR-001) | v1 실제 구현 |
> |------|---------------------|-------------|
> | AccessToken | 15분 | 15분 (동일) |
> | RefreshToken | UUID, 7일, Redis 저장 | **미구현** |
> | 로그아웃 처리 | Redis에서 RefreshToken 삭제 | 클라이언트 토큰 폐기만 수행 |
>
> **제외 이유**: 3주 개발 일정 내에서 RefreshToken 블랙리스트 관리 로직(로그아웃 시 무효화, 탈취 대응, Redis TTL 동기화)의 구현 복잡도가 범위를 초과한다고 판단. AccessToken 만료시간(15분)으로 보안 위협 범위를 최소화하는 방향으로 스코프를 조정.
>
> **v2에서 재도입 예정**:
> - RefreshToken 발급 및 Redis 저장 (`refresh:{token}` → `memberId`, TTL 7일)
> - 토큰 재발급 API (`POST /auth/reissue`) 추가
> - 로그아웃 시 Redis 삭제로 즉시 무효화
> - 탈취 감지 시 해당 토큰 블랙리스트 등록 검토
>
> **v1·v2 영향 받는 문서**: 04-기능명세서, 06-API명세서, 03-유스케이스(UC03·로그아웃), 11-패키지구조(AuthService)
